...
description: Use a set of network automation tools to accelerate data collection and
...
This post shares how I used a set of network automation tools to accelerate data collection and manipulation for the purposes of operating and troubleshooting both the old and new VPN environments.

All code can be found in its [Github repository](https://github.com/wollmannbruno/QuickWins/tree/master/CiscoASA/). It definitely helped me. Hopefully, it will help you too. I feel I need to disclose that this is only the second Python program I have ever written. If you have suggestions where some of the logic or code can be improved, I'm all ears. I will happily use any criticism to improve my skills.
...
![Output from the command show vpn-sessiondb anyconnect on a Cisco ASA firewall](images/Cisco_ASA_show_vpnsessiondb_anyconnect_CLI.png)

Without any filtering, fourteen lines of text are printed to the screen for every connected user. Depending on how many users are connected, you can expect tens of thousands or even hundreds of thousands of lines of output.
...
[Netmiko](https://pypi.org/project/netmiko/) is used to connect to devices, issue commands, and collect the output. [Openpyxl](https://pypi.org/project/openpyxl/) is used to manipulate an Excel workbook. Installation of both libraries is accomplished using the pip installer.
...
The main function is the hub of this program in that all other functions are called from it.
...
This function obtains credentials directly from user input. It also allows the user to quit the program gracefully by entering `q` or `Q` for either the username or password. This is useful if their credentials are rejected by a device. If an invalid username or password is entered, the program calls this function until the correct credentials are entered.
...
This function connects to the device and queries the VPN-session DB. It uses TextFSM and the NTC Templates to convert the output to structured data. It then returns the output to the `main()` function. If the program can not connect to the device, invalid user credentials are assumed, and the user is asked to re-enter credentials by calling `get_creds()` again.
...
- accepts the information from each device and saves it to a new tab in an Excel file.
  ...
  - If it does exist, a new tab is created in the first position.
...
- The remaining rows contain the dictionary data.
...
- Break it down into smaller functions.
  - It would be easier to read and simpler to follow.
- Remove dictionary ordering logic.
  - Because dictionaries are unordered, part of the logic is to make sure that the proper data is placed in the proper column. As of Python 3.7, dictionaries preserve the insertion order, making this code unnecessary.
...
The data collection process for any problems related to AnyConnect remote access VPN is now fully automated. Once this program completes, the network analyst can immediately zero in on the desired data. The human has been freed from the monotony of manually converting walls of text to useable information. The pantless home worker is that much closer to having their problem solved.
